...
/**
* @brief Checks if two vectors with std::complex items have the same items, even if not in the same order
* @param v1 First vector to check
* @param v2 Second vector to check
* @param t If true compares the real parts when sorting, if false compares the imaginary parts
* @return (bool) If they do or do not have the same items
**/
bool vequal(const std::vector<std::complex<double>> v1, const std::vector<std::complex<double>> v2, bool t) {
    std::vector<int> vv1, vv2, r;
    for (const auto& i: v1) {vv1.push_back((((t ? i.real() : i.imag()) >= 0) ? std::ceil(t ? i.real() : i.imag()) : std::floor(t ? i.real() : i.imag())));}
    for (const auto& i: v2) {vv2.push_back((((t ? i.real() : i.imag()) >= 0) ? std::ceil(t ? i.real() : i.imag()) : std::floor(t ? i.real() : i.imag())));}
    set::sort(vv1); set::sort(vv2); return set::sdiff(vv1, vv2).empty();
}
/**
...
int test_polynomial() {
    ...
    assert(((ss << ps).str() == std::string("x^2 + x + 1")) && "Error on stream operator!"); //"x^2 + x + 1"
    //--------------------------------------
    //                 MISC
    //--------------------------------------
    // Expression getter
    assert((ps.get_expression() == std::string("x^2 + x + 1")) && "Error on expression getter!");
    // Value getter
    assert((ps.get_value(1) == 3) && (ps.get_value(2) == 7) && "Error on value getter!");
    //--------------------------------------
    //               CALCULUS
    //--------------------------------------
    const Polynomial prr(std::map<int, Monomial>({{0, Monomial(-1, 0)}, {2, Monomial(1, 2)}})); //"x^2 - 1"
    const Polynomial prc(std::map<int, Monomial>({{0, Monomial(1, 0)}, {2, Monomial(1, 2)}})); //"x^2 + 1"
    // Derivative getter
    ...
    // Roots getter
    assert(vequal(prr.get_roots(), std::vector<std::complex<double>>({{1, 0}, {-1, 0}}), true) && "Error on real roots getter!");
    assert(vequal(prc.get_roots(), std::vector<std::complex<double>>({{0, 1}, {0, -1}}), false) && "Error on complex roots getter!");
    // Well done!
