...
**/
std::stringstream& emth::operator<<(std::stringstream& ss, const emth::Monomial& m) {
    if (m.coeff != 1.0 || m.degree == 0) {ss << m.coeff;} //if (m.coeff > 0) {ss << "+ ";} else {ss << "- ";}
    switch (m.degree) {
        case 0: break;
        case 1: ss << "x"; break;
        default: ss << "x^" << m.degree; break;
    } return ss;
}
/**
...
**/
std::ostream& emth::operator<<(std::ostream& out, const emth::Monomial& m) {
    if (m.coeff != 1.0 || m.degree == 0) {out << m.coeff;} //if (m.coeff > 0) {out << "+ ";} else {out << "- ";}
    switch (m.degree) {
        case 0: break;
        case 1: out << "x"; break;
        default: out << "x^" << m.degree; break;
    } return out;
}
/**
...
std::stringstream& emth::operator<<(std::stringstream& ss, const emth::Polynomial& p) noexcept {
    std::reverse_iterator<std::map<const int, emth::Monomial>::const_iterator> it = p.monomials.crbegin(); ss << it++->second;
    for(std::reverse_iterator<std::map<const int, emth::Monomial>::const_iterator> _it = it; _it != p.monomials.crend(); _it++) {
        if (_it->second.get_coeff() > 0) {ss << " + "; ss << _it->second;} else {ss << " - "; ss << _it->second.get_expression().erase(0, 1);}
    }
    return ss;
}
/**
...
std::ostream& emth::operator<<(std::ostream& out, const emth::Polynomial& p) noexcept {
    std::reverse_iterator<std::map<const int, emth::Monomial>::const_iterator> it = p.monomials.crbegin(); out << it++->second;
    for(std::reverse_iterator<std::map<const int, emth::Monomial>::const_iterator> _it = it; _it != p.monomials.crend(); _it++) {
        if (_it->second.get_coeff() > 0) {out << " + "; out << _it->second;} else {out << " - "; out << _it->second.get_expression().erase(0, 1);}
    }
    return out;
}
/**
...
**/
bool emth::Polynomial::is_empty() const noexcept {return this->monomials.empty();}
/**
* @brief Function for appending a lvalue monomial to the Polynomial
* @param m The Monomial object to append.
* @return (bool) If the operation was successful.
**/
bool emth::Polynomial::push_monomial(const emth::Monomial& m) noexcept {return this->push_monomial(emth::Monomial(m));};
/**
...
* @details Does not handle the edge case when called with a constant polynomial (polynomial of degree zero).
* @return (std::vector<double>) All the roots as doubles in a std::vector
**/
std::vector<std::complex<double>> emth::Polynomial::get_roots() const noexcept {
    Eigen::VectorXd cfs(this->get_degree()+1); for(const std::pair<const int, emth::Monomial>& kv: this->monomials) {cfs[kv.first] = kv.second.get_coeff();}
    const Eigen::PolynomialSolver<double, Eigen::Dynamic>::RootsType rts =  Eigen::PolynomialSolver<double, Eigen::Dynamic>(cfs).roots();
    std::vector<std::complex<double>> roots; for(const auto& rt: rts) {roots.push_back(std::complex(rt.real(), rt.imag()));};
    return roots;
}
/**
* @brief The swap function for the copy-and-swap idiom implemented by the assigment operators
